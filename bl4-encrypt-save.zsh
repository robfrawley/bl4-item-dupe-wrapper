...
function main() {
    ...
    write_header "ENCRYPTING SAVE"

    if ! python "${call_file}" encrypt -in "${yaml_file}" -out "${save_file}.new" -id "${STEAM_ID}"; then
        rm "${save_file}.new"
        write_error 'Failed to execute external command to encrypt save file ("%s").' "${save_file}"
        return 1
    fi
